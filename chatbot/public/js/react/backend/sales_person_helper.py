...
class ItemQuery:
    ...
    def __init__(self,session_id):
        ...
        # parameters
        frappe.errprint(frappe.conf.get("redis_cache"))
        message_history = RedisChatMessageHistory(
            session_id=session_id,
            url=frappe.conf.get("redis_cache") or "redis://localhost:6379/0",     
              
            )
        frappe.errprint(message_history)
        # self.memory = ConversationBufferMemory(
        #     memory_key="chat_history",
        #     return_messages=True,
        #     chat_memory = message_history,
        # )
        frappe.errprint("BEFORE SELF.MEMORY")
        ...
        frappe.errprint("AFTER SELF.MEMORY")
        frappe.errprint("BEFORE SELF.LLM")
        ...
        frappe.errprint("After SELF.LLM")
        ...
        frappe.errprint("SELF TOOL------"+str(self.tools))
        self.agent = self.initialize_agent()
        frappe.errprint("SELF AGENT-----"+str(self.agent))
    ...
    def get_stock_details(self,stock_inquiry_string):
        print("Item----------------",stock_inquiry_string)
        item = stock_inquiry_string.strip()
        frappe.errprint("Item-----"+str(item))
        sql_data = frappe.db.sql("""SELECT it.item_code,it.item_name,bn.actual_qty,bn.warehouse FROM `tabBin` bn inner join `tabItem` it on it.name = bn.item_code WHERE it.item_code like %s limit 2""",('%' + item + '%'),as_dict = 1)
        frappe.errprint(sql_data)
        return f"""
    ...
    def get_price_details(self,price_inquiry_string):
        ...
        sql_data = frappe.db.sql("""SELECT ip.item_code,ip.item_name,ip.price_list_rate FROM `tabItem Price` ip WHERE ip.item_code like %s and ip.selling = 1 limit 1""",('%' + item + '%'),as_dict = 1)
    ...
    def get_regionwise_sales_details(self, json_request: str):
        ...
        print("ARGUMENT DICTIONARY-------",data)

        up_sales_channel = data["up_sales_channel"]
        transaction_date = data["transaction_date"]
        print('sc', up_sales_channel)
    ...
    def run(self, userinput):
        frappe.errprint("CALLING RUN METHOD")
...
def get_sales_data(customer_name):
    ...
    select so.customer_name,so.transaction_date,soi.item_name,soi.qty,soi.rate,soi.amount from `tabSales Order` so inner join `tabSales Order Item` soi on so.name = soi.parent where so.docstatus = 1 and so.customer like %s limit 1
...
def get_item_details():
    ...
    select item_code,item_name,brand,item_group from `tabItem` where disabled = 0 limit 1
    """,as_dict = 1)
    return item_detail

def get_outstanding_amount(customer):
    outstanding_invoices = frappe.db.sql("""
    select si.customer,si.posting_date,si.due_date,si.outstanding_amount from `tabSales Invoice` si where si.outstanding_amount > 0 and si.customer like %s limit 1
    """,('%' + customer + '%'),as_dict = 1)
    return outstanding_invoices

def get_credit_limit_from_db(customer):
    credit_limit = frappe.db.sql("""
    select ccl.credit_limit from `tabCustomer Credit Limit` ccl where ccl.parent like %s limit 1
    """,('%' + customer + '%'),as_dict = 1)
    frappe.errprint("Credit limit------"+str(credit_limit))
